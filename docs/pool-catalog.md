...
- **Sidebar gauche (sticky)** : filtres par checkboxes — Catégories de piscine et Dimensions
...
> Le catalogue utilise principalement l'**image à la une** (featured image) de chaque post `piscine` et les taxonomies `categorie_piscine` / `dimension_piscine`.
...
### 3. Filtres combinés
Les filtres catégorie et dimension sont combinés en logique ET (`tax_query` avec `relation => AND`) côté serveur.
...
- Image à la une (ou placeholder si absente)

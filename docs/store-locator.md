...
- **Gauche (40%)** : filtres (recherche + select province) + liste de cartes groupées par province
- **Droite (60%)** : carte interactive Leaflet en `position: sticky`
...
| Fichier | Rôle |
|---|---|
| `store-locator.php` | Shortcode WordPress : récupère les données ACF, enqueue les assets, génère le HTML |
| `store-locator.js` | Logique JS : initialisation Leaflet, rendu dynamique, filtrage texte, recherche de proximité (géocodage Nominatim) |
| `store-locator.css` | Tous les styles : layout, cartes, marqueurs, popups, responsive |
...
### 7. Marqueurs numérotés
Les marqueurs sur la carte sont des cercles numérotés (`1, 2, 3...`) correspondant à l'ordre dans la liste.

### 8. Popups
Chaque marqueur a une popup affichant : nom, adresse, téléphone et un bouton « Y aller » (Google Maps).

### 9. Responsive
- **Desktop** : sidebar à gauche (40%), carte sticky à droite (60%)
...
La couleur primaire utilisée partout est `#1a4759`. Pour changer le thème de couleur, remplacer toutes les occurrences de cette valeur dans `store-locator.css` et `store-locator.js` (couleur des marqueurs).
